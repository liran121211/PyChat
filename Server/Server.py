...
class MultipleTCP:
    ...
    def client_transmission(self, client_socket, message):
        ...
        if cmd == "CHAT_ROOMS_LIST":
            query_result = self.database.query(table="rooms", filter_key=None, filter_value=None, column="name")
            rooms_list = [room_name[0] for room_name in query_result]
            client_socket.send(build_message(PROTOCOLS["chat_rooms_list"], "#".join(rooms_list)).encode())
    ...
    def online_users(self):
        while True:
            ...
            if self.database.conn is not None:
                if self.database.conn.closed == 0:
                    query_result = self.database.query(table="users", filter_key="online", filter_value=True)
                    online_user_list = [username[1] for username in query_result]
                    ...
                    for available_client in self.ready_to_write:
                        ...
                        if available_client.fileno() != -1:
                            available_client.send(build_message(PROTOCOLS["online_users"], "#".join(online_user_list)).encode())
            time.sleep(2)
